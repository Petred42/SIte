...
urlpatterns = [
    ...
    path('events/event/<int:pk>/speakers/<int:art>/reject', user_views.article_reject),  # отменить участие статьи
    path('events/event/<int:pk>/speakers/<int:art>/approve', user_views.article_approve),  # одобрить статью
    path('events/event/<int:pk>/speakers/<int:art>', user_views.article_speaker),  # подать заявку
